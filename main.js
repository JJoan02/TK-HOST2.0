...
let isInit = true;
let handlerFunction, participantsUpdateFunction, groupsUpdateFunction, deleteUpdateFunction;

async function loadHandlerModule() {
  const module = await import('./handler.js');
  handlerFunction = module.handler;
  participantsUpdateFunction = module.participantsUpdate;
  groupsUpdateFunction = module.groupsUpdate;
  deleteUpdateFunction = module.deleteUpdate;
}
...
@desc`;
  ...
  // Asigna las funciones correctamente y utiliza bind
  conn.handler = handlerFunction.bind(conn);
  conn.participantsUpdate = participantsUpdateFunction.bind(conn);
  conn.groupsUpdate = groupsUpdateFunction.bind(conn);
  conn.onDelete = deleteUpdateFunction.bind(conn);
...
global.reload = async (_ev, filename) => {
  if (pluginFilter(filename)) {
    ...
    if (filename in global.plugins) {
      if (existsSync(dir)) console.log(`Re - require plugin '${filename}'`);
      else {
