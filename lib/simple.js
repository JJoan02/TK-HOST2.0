            ...
            async value(m) {
                ...
                switch (m.messageStubType) {
                    case WAMessageStubType.REVOKE: {
                        // Maneja la revocación de un mensaje
                        emitGroupUpdate({ revoke: m.messageStubParameters[0] });
                        break;
                    }
                    case WAMessageStubType.GROUP_CHANGE_INVITE_LINK: {
                        // Maneja el cambio en el enlace de invitación del grupo
                        emitGroupUpdate({ inviteLink: m.messageStubParameters[0] });
                        break;
                    }
                    case WAMessageStubType.GROUP_CHANGE_ICON: {
                        // Maneja el cambio en el ícono del grupo
                        emitGroupUpdate({ icon: m.messageStubParameters[0] });
                        break;
                    }
                    default: {
                        // Maneja casos no previstos o desconocidos
                        console.log({
                            messageStubType: m.messageStubType,
                            messageStubParameters: m.messageStubParameters,
                            type: WAMessageStubType[m.messageStubType] // Esto ayuda a identificar el tipo exacto en caso de error
                        });
                        break;
                    }
                }
