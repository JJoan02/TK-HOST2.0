...
const smsAvisoRG = () => { return `╰⊱✅⊱ *RESULTADO* ⊱✅⊱╮\n\n¡Sorprendentemente, algo salió bien! 🎉📈` } 
const smsAvisoAG = () => { return `╰⊱⚠️⊱ *ADVERTENCIA* ⊱⚠️⊱╮\n\n¡Oh, oh! Parece que alguien metió la pata. 😬⚠️` }
const smsAvisoIIG = () => { return `╰⊱❕⊱ *INFORMACIÓN* ⊱❕⊱╮\n\n¡Oye, presta atención! Información crucial aquí. 🧐📚` }
const smsAvisoFG = () => { return `╰⊱❌⊱ *ERROR* ⊱❌⊱╮\n\n¡Bingo! Encontraste el error. 🙄❌` }
const smsAvisoMG = () => { return `╰⊱❗️⊱ *ACCIÓN MAL USADA* ⊱❗️⊱╮\n\n¡Bravo! Hiciste algo mal. 🙃❗️` }
const smsAvisoEEG = () => { return `╰⊱📩⊱ *REPORTE* ⊱📩⊱╮\n\n¡Alerta de reporte! Mira esto antes de que sea tarde. 📝📬` }
const smsAvisoEG = () => { return `╰⊱💚⊱ *ÉXITO* ⊱💚⊱╮\n\n¡Milagro! Algo salió bien. 🥳💚` }

const smsRowner = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO EL NÚMERO DEL BOT PUEDE USAR!! 🤖\`\`\`` }
const smsOwner = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO MI CREADOR(A) LO PUEDE USAR!! 🛠️ Porque claro, nadie más tiene ese privilegio. 🙄\`\`\`` }
const smsMods = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO MODERADORES Y MI CREADOR(A) LO PUEDEN USAR!! 🛡️ Sí, porque ellos son los VIP aquí. 🙄\`\`\`\`` }
const smsPremium = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO ESTÁ DISPONIBLE PARA USUARIOS PREMIUM Y MI CREADOR(A)!! PARA SER PREMIUM COMPRE UN PASE USANDO #pass premium 💳✨ ¡Porque nada es gratis! 🙃\`\`\`` }
const smsGroup = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO SE PUEDE UTILIZAR EN GRUPOS!! 👥 ¡Así que consigue amigos! 🙄\`\`\`` }
const smsPrivate = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO SE PUEDE UTILIZAR AL PRIVADO!! 🔒 ¡No seas tan público! 🙄\`\`\`` }
const smsAdmin = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO ES PARA ADMINS!! 👑 ¡Claramente necesitas más poder! 😏\`\`\`` }
const smsBotAdmin = () => { return `\`\`\`¡¡NECESITO SER ADMIN PARA QUE PUEDAS USAR ESTE COMANDO!! 🤖👑 ¡Dame mi corona primero! 🙄\`\`\`` }
const smsUnreg = () => { return `\`\`\`¡¡NECESITAS ESTAR REGISTRADO(A) PARA USAR ESTE COMANDO, ESCRIBE #verificar PARA REGISTRARTE!! 📋✅ ¡Vamos, no seas tímido! 🤨\`\`\`` }
const smsRestrict = () => { return `\`\`\`¡¡ESTE COMANDO ESTÁ RESTRINGIDO POR MI CREADORA(A)!! 🚫 ¡Porque las reglas son reglas! 🙃\`\`\`` }

//main.js
const smsWelcome = () => { return `*╭┈⊰* @subject *⊰┈ ✦*\n*┊✨ ¡BIENVENIDO(A) AL CAOS!*\n┊💖 @user\n┊📄 *LEA LA DESCRIPCIÓN DEL GRUPO, SI TIENE TIEMPO...* 🤓\n*╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈ ✦*\n${String.fromCharCode(8206).repeat(850)}\n@desc`}
const smsBye = () => { return '*╭┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈⊰*\n┊ *@user*\n┊ *NO FUE DIGNO(A) DE ESTAR AQUÍ!!* 🌟\n*╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈⊰* 👋😆' }
const smsSpromote = () => { return '*@user HA SIDO ASCENDIDO COMO ADMIN DEL GRUPO!! 🎉 Ahora tienes más trabajo, ¡felicidades! 🥳*' }
const smsSdemote = () => { return '*@user HA SIDO DEGRADADO COMO ADMIN DEL GRUPO!! 😬 De vuelta al rincón, amigo. 😅*' }
const smsSdesc = () => { return '*CONOZCAN LA NUEVA DESCRIPCIÓN DEL GRUPO:*\n\n@desc 📄 Porque todos leemos esto, ¿verdad? 😏' }
const smsSsubject = () => { return '*SE HA ACTUALIZADO EL NOMBRE DEL GRUPO!!*\n@subject 📝 Nuevo nombre, mismas locuras. 🤪' }
const smsSicon = () => { return '*SE HA ACTUALIZADO LA FOTO DEL GRUPO!!* 📸 ¡Esperemos que sea decente! 😂' }
const smsSrevoke = () => { return '*CONOZCAN EL NUEVO ENLACE DEL GRUPO!!*\n*@revoke* 🔗 ¡Enlace fresquito, recién sacado del horno! 🥖' }
const smsConexion = () => { return `\n╭┈ ┈ ┈ ┈ ┈ • ${packname} • ┈ ┈ ┈ ┈ ┈╮\n┊ 💚 CONEXIÓN EXITOSA CON WHATSAPP 💚\n╰┈ ┈ ┈ ┈ ┈ ┈ ┈ ┈ ┈ ┈ ┈ ┈ ┈ ┈ ┈ ┈ ┈ ┈ ┈ ┈ ┈ ┈╯ 🥳` }
const smsCargando = () => { return `✨ CARGANDO... 🕒 Ten paciencia, Roma no se construyó en un día...` }
...
const smsCodigoQR = () => { return `\n✅ ESCANEA EL CÓDIGO QR EXPIRA EN 45 SEGUNDOS ✅ ¡Corre que se va! 🏃‍♂️` }
const smsConexionOFF = () => { return `\n⚠️ SIN CONEXIÓN, BORRE LA CARPETA ${global.authFile} Y ESCANEA EL CÓDIGO QR ⚠️ Parece que alguien no pagó la factura del internet... 🙄` }
...
const smsConexionreem = () => { return `\n⚠️ CONEXIÓN REEMPLAZADA, SE HA ABIERTO OTRA NUEVA SESIÓN, POR FAVOR, CIERRA LA SESIÓN ACTUAL PRIMERO. 🙄🔄` }
const smsConexionreinicio = () => { return `\n❇️ CONECTANDO AL SERVIDOR... 📡 ¡Crucemos los dedos! 🤞` }
const smsConexiontiem = () => { return `\n⌛ TIEMPO DE CONEXIÓN AGOTADO, RECONECTANDO... 🕰️` }
const smsConexiondescon = (reason, connection) => { return `\n⚠️❗ RAZÓN DE DESCONEXIÓN DESCONOCIDA: ${reason || ''} >> ${connection || ''} 🚫🔌` }
const languageSave = () => { return `✅ El idioma ya está configurado. 🎉🗣️` }
const languageRegister = (selectedLanguage) => { return `❇️ Se ha configurado el idioma como "${selectedLanguage}". 🌐🗣️` }
...
const smsContacto4 = () => { return 'Contacto Oficial de Admin-TK 🔰'}
...
//_allantilink.js 
const smsTextoYT = () => { return '🔰 𝗦𝘂𝗽𝗲𝗿 𝗔𝗱𝗺𝗶𝗻-𝗧𝗞 - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 '} 
