...
const smsWelcome = () => { 
    return `
✨ *¡Bienvenid@ a @subject, @user! 🎉*

Nos alegra tenerte con nosotros. Para que podamos conocerte mejor, por favor preséntate con tu nombre y una foto.

📢 *Canal TK:*
> [Únete aquí](https://whatsapp.com/channel/0029VaS4zeE72WTyg5et571r)
...
const smsBye = () => { 
    const messages = [
        "No todos pueden manejar tanta grandeza. 🙃",
        "Esperamos que encuentres un lugar más adecuado para ti... ¡Buena suerte! 😂",
        "Algunos simplemente no están hechos para esto. 🤷‍♂️",
        "¡Hasta nunca, campeón! 🏆",
        "Que la fuerza te acompañe... porque la necesitarás. 😜",
        "Adiós y gracias por nada. 😏",
        "No llores porque se acabó, sonríe porque te vas. 😆",
        "Menos mal que te vas, ahora hay más espacio para nosotros. 😜",
        "Fue un placer... mentira, ni te conocemos. 👋",
        "La puerta es ancha, úsala bien. 🚪",
        "Que encuentres lo que buscas, y si no, al menos lo intentaste. 🤞",
        "Otro menos, ¡seguimos siendo increíbles! ✨",
        "Suerte allá afuera, la necesitarás. 🐾",
        "Recuerda, la grandeza no es para todos. 👑",
        "¡Adiós! No te preocupes, apenas notaremos tu ausencia. 😜",
        "¡Buena suerte en tus futuras mediocridades! 😏"
    ];
...
const smsPrivadoDonar = () => { 
    return `
🍄 *_También puedes compartir lo siguiente como Muestra de Apoyo_*
*_Califica el repositorio con una 🌟 Gracias!!_*
https://github.com/JJoan02/Admin-TK
`;
}
...
*${m.messageStubParameters[0]}* 🥳 ¡Qué original!
`;
};
...
*${groupMetadata.subject}* 📸 ¡Un cambio revolucionario!
`;
};
...
${inf}*» ${usuario}*
*HA PERMITIDO QUE ${m.messageStubParameters[0] == 'on' ? 'SOLO ADMINS' : 'TODOS'} PUEDAN CONFIGURAR ${groupMetadata.subject}* 🛠️ ¡Democracia en acción!
`;
};
...
${inf}*EL ENLACE DE ${groupMetadata.subject} HA SIDO RESTABLECIDO POR:*
*» ${usuario}* 🧙‍♂️✨ ¡Magia pura!
`;
};
...
${inf}*${groupMetadata.subject} HA SIDO ${m.messageStubParameters[0] == 'on' ? 'CERRADO 🔒' : 'ABIERTO 🔓'} POR ${usuario}* 🤷‍♂️ ¡Qué decisión tan audaz!

💬 *AHORA ${m.messageStubParameters[0] == 'on' ? 'SOLO ADMINS' : 'TODOS'} PUEDEN ENVIAR MENSAJES* 🗣️ ¡Hablemos todos a la vez!
`;
};
...
${inf}*@${m.messageStubParameters[0].split`@`[0]} AHORA ES ADMIN EN*
*» ${groupMetadata.subject}* 👑 ¡Poder para los poderosos!

✨ *ACCIÓN HECHA POR:*
*» ${usuario}* 🎩 ¡Qué noble gesto!
`;
};
...
${inf}*@${m.messageStubParameters[0].split`@`[0]} DEJA DE SER ADMIN EN*
*» ${groupMetadata.subject}* 🚪 ¡Adiós, poder!

✨ *ACCIÓN HECHA POR:*
*» ${usuario}* 👋 ¡Hasta la vista, baby!
`;
};
