...
if (!opts['test']) {
  if (global.db) {
    setInterval(async () => {
      ...
      if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', 'serbot'], tmp.forEach((filename) => spawn('find', [filename, '-amin', '3', '-type', 'f', '-delete'])))
    }, 60 * 1000)
  }
}
...
async function clearTmp() {
  ...
  return filename.map(file => {
    ...
    if (stats.isFile() && (Date.now() - stats.mtimeMs >= 1000 * 60 * 3)) {
      if (existsSync(file)) unlinkSync(file) // 3 minutes
    }
    return false
  })
}
...
function purgeSessionSB() {
  try {
    ...
    listaDirectorios.forEach(directorio => {
      const dirPath = `../serbot/${directorio}`
      if (statSync(dirPath).isDirectory()) {
        ...
        DSBPreKeys.forEach(fileInDir => {
          let filePath = `../serbot/${directorio}/${fileInDir}`
...
function purgeOldFiles() {
  ...
  directories.forEach(dir => {
    // Dado que 'readdirSync' no recibe callback, se quita esa parte.
