...
this.sendMessage(m.chat, {text: `${lenguajeGB['smsCont7']()} *${usedPrefix}buy*`,  contextInfo: {externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: gt, body: ' 👻 𝗔𝗱𝗺𝗶𝗻-𝗧𝗞 - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 ', previewType: 0, thumbnail: gataImg, sourceUrl: accountsgb }}}, { quoted: m })         
//this.reply(m.chat, `${lenguajeGB['smsCont7']()} *${usedPrefix}buy*`, m)
continue //Sin límite
}
if (plugin.level > _user.level) {
this.sendMessage(m.chat, {text: `${lenguajeGB['smsCont9']()} *${plugin.level}* ${lenguajeGB['smsCont10']()} *${_user.level}* ${lenguajeGB['smsCont11']()} *${usedPrefix}nivel*`,  contextInfo: {externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: gt, body: ' 👻 𝗔𝗱𝗺𝗶𝗻-𝗧𝗞 - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 ', previewType: 0, thumbnail: gataImg, sourceUrl: accountsgb }}}, { quoted: m })         
...
// Mapeo de palabras clave a listas de emojis
const emojiMappings = {
    'happy': ["😀", "😃", "😁", "😂", "🤣"],
    'sad': ["😞", "😔", "😟", "😢", "😭"],
    'angry': ["😠", "😡", "🤬"],
...
// Función para seleccionar un emoji basado en el texto del mensaje
function getEmojiForText(text) {
    for (const [keyword, emojisList] of Object.entries(emojiMappings)) {
        if (text.toLowerCase().includes(keyword)) {
            return pickRandom(emojisList);
        }
    }
...
// Función para seleccionar un emoji aleatorio de una lista
function pickRandom(list) {
    return list[Math.floor(Math.random() * list.length)];
}

// Manejar la reacción a mensajes
if (db.data.chats[m.chat].reaction && m.text.match(/(ción|dad|aje|oso|izar|mente|pero|tion|age|ous|ate|and|but|ify)/gi)) {
    // Seleccionar un emoji basado en el texto del mensaje usando la función inteligente
    let emot = getEmojiForText(m.text);
    if (!m.fromMe) {
