  ...
  try {
    ...
    for (let name in global.plugins) {
      ...
      if ((usedPrefix = (match[0] || '')[0])) {
        ...
        let keni = `${plugin.botAdmin || plugin.admin || plugin.group || plugin || noPrefix || hl || m.text.slice(0, 1) == hl || plugin.command}`;
        if (
          adminMode &&
          !isOwner &&
          !isROwner &&
        ...
        if (
          ...
          this.reply(
            m.chat,
            `Sus coins se acabaron, usa *${usedPrefix}claimcoins* para su recompensa diaria\n\no\nHabla con mi creador para obtener premium *${usedPrefix}owner*`,
            m
          );
        ...
        if (plugin.level > _user.level) {
          this.reply(
            m.chat,
            `✧ Necesitas estar en el nivel ${plugin.level} para usar este comando.\n*✧ Su nivel actualmente:* ${_user.level} 📊`,
            m
          );
    ...
    case 'add':
      ...
      if (chat.welcome) {
        ...
        for (let user of participants) {
          try {
            let pp =
              'https://telegra.ph/file/3067b920347facbb69bb1.jpg';
          } catch (e) {
          } finally {
            let textp = (
              action === 'add'
                ? (chat.sWelcome ||
                    this.welcome ||
                    (await Connection.conn.welcome) ||
                    'Bienvenido, @user!')
                    .replace('@subject', await this.getName(id))
                    .replace('@desc', groupMetadata.desc?.toString() || 'unknow')
                : (chat.sBye ||
                    this.bye ||
                    (await Connection.conn.bye) ||
                    'Bye bye, @user!')
...
async function groupsUpdate(groupsUpdate) {
  ...
  for (const groupUpdate of groupsUpdate) {
    ...
    if (groupUpdate.desc)
      text = (chats.sDesc || this.sDesc || (await Connection.conn.sDesc) || '```Description has been changed to```\n@desc').replace('@desc', groupUpdate.desc);
    if (groupUpdate.subject)
      text = (chats.sSubject || this.sSubject || (await Connection.conn.sSubject) || '```Subject has been changed to```\n@subject').replace('@subject', groupUpdate.subject);
    if (groupUpdate.icon)
      text = (chats.sIcon || this.sIcon || (await Connection.conn.sIcon) || '```Icon has been changed to```').replace('@icon', groupUpdate.icon);
    if (groupUpdate.revoke)
      text = (chats.sRevoke || this.sRevoke || (await Connection.conn.sRevoke) || '```Group link has been changed to```\n@revoke').replace('@revoke', groupUpdate.revoke);
    if (!text) continue;
...
global.dfail = (type, m, conn) => {
  ...
  let deco_msg =
    '`ׅㅤ ⵿͝ ፝֟͜口֟፝͜ ͝ ⵿࿙࿚ ⚠️  Admin TK ⚠️ ࿙࿚⵿۫ ͝ ፝֟͜口፝֟͜ ⵿͝*`\n' +
    `${msg}\n` +
...
export { handler, participantsUpdate, groupsUpdate, deleteUpdate };
